              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '800px' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                    ...
                    <th style={{ padding: '0.75rem 0.5rem', textAlign: 'right', color: 'var(--text-secondary)', fontSize: '0.8rem' }}>Price</th>
                ...
                <tbody>
                  {tradeBookCombinedData.tradeBookList.map((item: TradeBookListItem, index: number) => (
                    <tr key={index} style={{ borderBottom: index < tradeBookCombinedData.tradeBookList.length - 1 ? '1px solid rgba(255,255,255,0.03)' : 'none' }}>
                      <td style={{ padding: '0.5rem', textAlign: 'left' }}>{item.time}</td>
                      <td style={{ padding: '0.5rem', textAlign: 'right' }}>{item.price.toLocaleString()}</td>
                      <td style={{ padding: '0.5rem', textAlign: 'right' }}>{formatCompactNumber(item.buy_lot)}</td>
                      <td style={{ padding: '0.5rem', textAlign: 'right' }}>{formatCompactNumber(item.sell_lot)}</td>
                      <td style={{ padding: '0.5rem', textAlign: 'right', color: parseFloat(item.net_lot.replace(/,/g, '')) >= 0 ? 'var(--accent-success)' : 'var(--accent-warning)' }}>{formatCompactNumber(item.net_lot)}</td>
                      <td style={{ padding: '0.5rem', textAlign: 'right' }}>{formatCompactNumber(item.buy_frequency)}</td>
                      <td style={{ padding: '0.5rem', textAlign: 'right' }}>{formatCompactNumber(item.sell_frequency)}</td>
                      <td style={{ padding: '0.5rem', textAlign: 'right', color: parseFloat(item.net_frequency.replace(/,/g, '')) >= 0 ? 'var(--accent-success)' : 'var(--accent-warning)' }}>{formatCompactNumber(item.net_frequency)}</td>
                      <td style={{ padding: '0.5rem', textAlign: 'right' }}>{formatCompactNumber(item.buy_value)}</td>
                      <td style={{ padding: '0.5rem', textAlign: 'right' }}>{formatCompactNumber(item.sell_value)}</td>
                      <td style={{ padding: '0.5rem', textAlign: 'right', color: parseFloat(item.net_value.replace(/,/g, '')) >= 0 ? 'var(--accent-success)' : 'var(--accent-warning)' }}>{formatCompactNumber(item.net_value)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
